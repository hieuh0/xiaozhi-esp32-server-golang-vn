...
const audioBlobUrls = ref({}) // stores audio Blob URLs
...
const loadMessages = async () => {
  ...
  try {
    ...
    // Backend returns newest-first; reverse so newest appears at bottom
    const data = response.data.data || []
    messages.value = [...data].reverse() // reversed so newest appears at bottom
    total.value = response.data.total || 0
    
    // preload messages with audio
    await preloadAudioMessages()

    // scroll to bottom after loading (show latest messages)
    await nextTick()
    scrollToBottom()
  } catch (error) {
...
// Format short timestamp (for message bubbles)
...
// Get audio URL (using Blob URL to support auth)
const getAudioUrl = async (messageId) => {
  ...
  try {
    // Use axios to fetch audio data (automatically includes auth token)
    ...
    // Silent: log only, no error toast
...
const preloadAudioMessages = async () => {
  ...
  // Concurrent preloading with concurrency limit
