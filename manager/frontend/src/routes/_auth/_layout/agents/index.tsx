...
function AgentCard({ agent, onEdit, onHistory, onDevices, onDelete }: { agent: Agent; onEdit: () => void; onHistory: () => void; onDevices: () => void; onDelete: () => void }) {
  ...
  return (
    <article className="p-5 rounded-2xl bg-[var(--color-surface-1)] border border-[var(--color-line)] flex flex-col gap-3.5 max-w-[340px] w-full">
      <div className="flex items-center gap-3.5 min-w-0">
        <div className="w-12 h-12 rounded-2xl flex-none inline-flex items-center justify-center text-white bg-gradient-to-b from-blue-400 to-blue-600 shadow-[0_12px_24px_rgba(0,122,255,0.18)]">
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-md font-medium', memoryBadgeClass(agent.memory_mode))}>
          <Brain className="w-3 h-3" />
          {agent.memory_mode === 'long' ? t('memory_long') : agent.memory_mode === 'short' ? t('memory_short') : t('memory_none')}
        </span>
        <span className={cn(
          'inline-flex items-center gap-1 px-2 py-0.5 rounded-md font-medium border text-xs',
        ...
        {hasMcp(agent) && (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md font-medium border text-xs status-primary">
            <Plug className="w-3 h-3" />MCP
          </span>
        )}
        {isOpenClawActive(agent) && (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md font-medium border text-xs status-success">
      ...
      <div className="grid grid-cols-2 gap-2 mt-auto">
        <Button variant="outline" size="sm" className="text-xs font-semibold rounded-xl status-primary border" onClick={onEdit}>
          <Settings className="w-3.5 h-3.5 mr-1" />{t('config')}
        </Button>
        <Button variant="outline" size="sm" className="text-xs font-semibold rounded-xl" onClick={onHistory}>
          <MessageSquare className="w-3.5 h-3.5 mr-1" />{t('chat')}
        </Button>
        <Button variant="outline" size="sm" className="text-xs font-semibold rounded-xl" onClick={onDevices}>
          <Link2 className="w-3.5 h-3.5 mr-1" />{t('device')}
        </Button>
        <Button variant="outline" size="sm" className="text-xs font-semibold rounded-xl status-danger border" onClick={onDelete}>
...
function AgentsInner() {
  ...
  return (
    <div className="grid gap-5 p-6">
      <section className="flex flex-wrap items-center justify-between gap-4 px-6 py-4 rounded-2xl bg-[var(--color-surface-1)] border border-[var(--color-line)] shadow-sm">
      ...
      {agents.length === 0 ? (
        <div className="flex justify-center">
          <div className="p-8 rounded-2xl bg-[var(--color-surface-1)] border border-[var(--color-line)] text-center">
            <Monitor className="w-16 h-16 mx-auto text-[var(--color-primary)] mb-4" />
            <h3 className="text-2xl font-bold text-[var(--color-text)] mb-2.5">{t('create_first_agent')}</h3>
