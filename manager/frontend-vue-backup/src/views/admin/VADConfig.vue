...
const loadConfigs = async () => {
  loading.value = true
  try {
    const response = await api.get('/admin/vad-configs', { params: { page: page.value, page_size: pageSize.value } })
...
const editConfig = (config) => {
  ...
  try {
    ...
    if (configObj.webrtc_vad) {
      ...
      if (config.provider === 'webrtc_vad') form.webrtc_vad = { ...form.webrtc_vad, ...configObj }
      else if (config.provider === 'silero_vad') form.silero_vad = { ...form.silero_vad, ...configObj }
      else if (config.provider === 'ten_vad') form.ten_vad = { ...form.ten_vad, ...configObj }
    }
  } catch (error) {
...
const handleAction = (command, row) => {
  switch (command) {
    ...
    case 'test': testConfig(row, 'vad'); break
  ...
  <div class="grid gap-4">
    ...
    <Dialog v-model:open="showDialog">
      <DialogContent class="max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{{ editingConfig ? t('edit_vad_config') : t('add_vad_config') }}</DialogTitle>
        </DialogHeader>
        <div class="max-h-[65vh] overflow-y-auto pr-1">
          <VADConfigForm ref="formRef" :model="form" />
        </div>
        <DialogFooter>
